...
test_that("blocking large = FALSE", {
  ...
  p <- pairs_blocking(d1, d2, large = FALSE) 
  
  expect_equal(p$x, rep(1:4, 4))
  expect_equal(p$y, rep(1:4, each = 4))
  
  
  p <- pairs_blocking(d1, d2, blocking_var = "a", large = FALSE)
